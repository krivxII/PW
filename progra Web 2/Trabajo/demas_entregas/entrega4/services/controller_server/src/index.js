...
const rutaG = process.env.G_SERVER_URL || "http://localhost:4000/" ; 
const rutaE = process.env.E_SERVER_URL || "http://localhost:8085"
...
async function CrearUsuario(usuario,contraseña) {
  ...
  const response = await fetch(rutaG, {
    method: 'POST',
    headers: {
      "Content-Type": "application/json"
    },
...
async function ObtenerUsuario(usuario,token="nono") {
  ...
  const response = await fetch(rutaG, requestOptions).then(res => res.json().then(data => ({ ok: res.ok, status: res.status, body: data })));
...
async function flip(carta, SuperState) {
  ...
  const response = await fetch(rutaE+"/flip", requestOptions)
...
async function flipAllCards(SuperState, ) {
  let headers= {
    "Content-Type": "application/json"
  }
 ...
 const response = await fetch(rutaE+"/flipall", requestOptions)
...
async function guardarPartida(Props) {
    ...
    const response =  await  fetch(rutaG, requestOptions)
