...
let raw = JSON.stringify({usuario,ganadas,perdidas,partidas});
  ...
  const response =  await fetch(ruta+"/modificarUsuario", requestOptions).then(res => res.json().then(data => ({ ok: res.ok, status: res.status, body: data })));
...
async function ObtenerUsuario(usuario, auth, history,token="nono") {//listo no se usa
  ...
  const response = await fetch(ruta+"/obtenerUsuario", requestOptions).then(res => res.json().then(data => ({ ok: res.ok, status: res.status, body: data })));
...
async function CrearUsuario(nombre,usuario,contraseña, auth, history ) {//nuevo funcional 
  ...
  const response =  await fetch(ruta+"/registrarUsuario", requestOptions).then(res => res.json().then(data => ({ ok: res.ok, status: res.status, body: data })));
